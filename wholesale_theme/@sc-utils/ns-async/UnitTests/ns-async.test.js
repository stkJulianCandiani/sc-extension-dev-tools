...
describe('ns-async', () => {
    describe('waterfall', () => {
        it('should call final callback when functions array is empty', () => {
        ...
        it('should call functions forwarding data', () => {
            const fn1 = jest.fn((data, cb) => cb(null, data));
            const fn2 = jest.fn((data, cb) => cb(null, data));
            const callback = jest.fn();
            ...
            expect(fn1).toHaveBeenCalledWith(data, expect.anything());
            expect(fn2).toHaveBeenCalledWith(data, expect.anything());
            expect(callback).toHaveBeenCalled();
        });

        it('should call functions forwarding data when no callback is provided', () => {
            const fn = jest.fn((data, cb) => cb(null, data));
            const data = { some: "data" };
            ...
            expect(fn).toHaveBeenCalledWith(data, expect.anything());
        });
