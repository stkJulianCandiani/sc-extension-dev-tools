    ...
    <div class="locator-venue-details-container">
        {{#if showAddress}}
            <div class="locator-venue-details-container-address">
                <h4 class="locator-venue-details-title">{{location.name}}</h4>
                {{#if showStoreAddress}}
                ...
                {{#if corporateWebsite}}
                    <a class="store-locator-details-corporate-website-link" target="_blank" href={{corporateWebsite}}>{{translate 'Corporate Web Site'}}</a>
                {{/if}}
                {{#if shoppingWebsite}}
                    <a class="store-locator-details-shopping-website-link" target="_blank" href={{shoppingWebsite}}>{{translate 'Shopping Web Site'}}</a>
                {{/if}}
                {{#if showPhone}}
                    <p>
                ...
                {{/if}}
                {{#if showDescription}}
                    {{#each locationDescription}}
                        </br>

                        <p>
                            {{#each lines}}
                                <span class="locator-venue-details-description">
                                    {{this}}
                                </span>
                            {{/each}}
                        </p>
