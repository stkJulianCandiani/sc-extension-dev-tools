...
const fs = require('fs');
const nsServer = require('@sc-utils/ns-server');
...
describe('NS Server', () => {
    describe('Create a server with middleware callback', () => {
        ...
        it('http.listen to be called when server.listen is called', () => {
            const server = nsServer.createServer();
            server.listen();
        ...
        it('http.close to be called when server.close is called', () => {
            const server = nsServer.createServer();
            server.close();
    ...
    describe('Check status of the response', () => {
        ...
        it('Check 200 jpg', async () => {
            ...
            expect(httpServer.getData('write')).toEqual([`Data of ${path.join("C:\\MyDirectory", "file.jpg")}`]);
            expect(httpServer.getData('end')).toBe(true);
        });
        ...
        it('Check 200 gif response send file', async () => {
            ...
            expect(httpServer.getData('write')).toEqual([`Data of ${path.join("C:\\MyDirectory", "file.wav")}`]);
            expect(httpServer.getData('end')).toBe(true);
        ...
        it('Check 200 multiple urls', async () => {
            ...
            expect(httpServer.getData('write')).toEqual([
                `Data of ${path.join("C:\\RootDirectory", "test13", "index.html")}`
            ]);
            expect(httpServer.getData('end')).toBe(true);
